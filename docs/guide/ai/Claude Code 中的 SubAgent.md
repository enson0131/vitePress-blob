...
单一 Agent 架构是最简单的架构，只有一个 Agent，所有的任务都由这个 Agent 完成。

#### 2. 单 Agent + Skills

多 Agent 架构是多个 Agent 协同工作，完成一个任务。
...
如今市面上会有很多创建 SubAgent 的工具，SKILL 也好模版也好，但如何判断这个 SubAgent 是否好，不仅需要有大量的实践经验，也需要有同学们对 SubAgent有体系化的认知，有自己的判断标准。